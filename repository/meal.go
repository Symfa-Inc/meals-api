...
func (m MealRepo) Get(mealDate time.Time, id, clientID string) ([]swagger.GetMeal, int, error) {
	var meals []domain.Meal
	var mealsResponse []swagger.GetMeal
	...
	for _, meal := range meals {
		...
		mealsResponse = append([]swagger.GetMeal{mealDishes}, mealsResponse...)
	}

	if mealsResponse == nil {
		//return nil, http.StatusNotFound, errors.New("meal for current day not found")
		mealsResponse = make([]swagger.GetMeal, 0)
	}
...
// Get returns list of meals, total items if and error
func (m MealRepo) GetByRange(startDate time.Time, endDate time.Time, id, clientID string) ([]swagger.GetMeal, int, error) {
	var meals []domain.Meal
	var mealsResponse []swagger.GetMeal
